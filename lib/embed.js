...
function readMultimodalConfig() {
	...
	// v5: reach Harper APIs through the `harper` package export rather than the
	// `globalThis` bindings — under the VM module loader an application gets its
	// own module cache and globals are not a reliable surface. `server.config` is
	// populated during boot and this runs per-request, so the read is never ahead
	// of the assignment.
	const cfg = server.config?.models?.embedding?.multimodal
